...
import { FolderKanban, FileText, Receipt, MessageSquare, ArrowRight, Plus, GraduationCap, CalendarClock, UsersRound } from "lucide-react";
...
export default async function DashboardHome() {
  ...
  const [projects, quotes, invoices, unreadCount, lastMessages, enrollment, teamCount] = await Promise.all([
    ...
    isEntreprise ? db.teamMember.count({ where: { ownerId: clientId } }) : Promise.resolve(0),
  ]);
  ...
  const nextDeadline = projects
    .filter((p) => p.deadline && new Date(p.deadline) >= new Date() && p.status !== "DELIVERED" && p.status !== "CANCELLED")
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="mb-6 overflow-hidden rounded-3xl bg-forest-900 p-6 text-cream-100 sm:p-8">
        ...
        <div className="mt-5 flex flex-wrap gap-2.5">
          ...
          <Link href="/dashboard/devis" className="inline-flex items-center gap-1.5 rounded-full border border-forest-700 px-4 py-2 text-xs font-semibold text-cream-100 transition hover:bg-forest-800">
            <FileText className="h-3.5 w-3.5" aria-hidden="true" />
            Mes devis {pendingQuotes > 0 && `(${pendingQuotes})`}
          </Link>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <StatCard
          ...
          value={nextDeadline ? formatShortDate(nextDeadline.deadline!) : "—"}
          hint={nextDeadline ? nextDeadline.title : "Aucune échéance à venir"}
          accent="terra"
        />
      </div>
      ...
      {activeProjects.length > 0 && (
        <div className="mt-4 rounded-3xl border border-cream-300 bg-card p-6 shadow-card">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-display text-lg font-semibold text-ink-900">Avancement de mes projets</h2>
            <Link href="/dashboard/projets" className="flex items-center gap-1 text-xs font-semibold text-terra-600 hover:underline">
              ...
              <div key={p.id}>
                <div className="mb-1.5 flex items-center justify-between gap-3">
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    ...
                    <span className="text-xs font-semibold text-ink-500">{p.progress}%</span>
                  </div>
                </div>
                <div className="h-2 overflow-hidden rounded-full bg-cream-200">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-terra-500 to-gold-400 transition-all"
      ...
      {isEntreprise && (
        <Link
          ...
          className="mt-4 flex items-center gap-4 rounded-2xl border border-gold-300/60 bg-gold-100/40 p-4 transition hover:bg-gold-100/70"
        >
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gold-500 text-forest-900">
            <UsersRound className="h-5 w-5" aria-hidden="true" />
          </span>
          <span className="min-w-0 flex-1">
            <span className="block text-sm font-semibold text-ink-900">Mon équipe</span>
            <span className="mt-0.5 block text-xs text-ink-500">
              {teamCount === 0 ? "Invitez vos collaborateurs à rejoindre votre espace entreprise." : `${teamCount} membre(s) invité(s) ou actif(s)`}
            </span>
          </span>
